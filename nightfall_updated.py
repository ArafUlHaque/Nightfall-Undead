...
ult_anim_timer = 0
ult_fires = []
ULT_MAX_COOLDOWN = 3600  # 60 seconds (60 frames * 60s)

# Rain & Grass
rain_drops = []   
grass_blades = []  # Added for 3D grass

# Generate Random Grass
for _ in range(3000):
    grass_blades.append((random.uniform(-GRID, GRID), random.uniform(-GRID, GRID), random.uniform(3, 8)))
...
def draw_boss_lightning():
    boss_active = any(e['kind'] in ('boss1', 'boss2') for e in enemies)
    if boss_active and random.random() < 0.1:
        glPushMatrix()
        glLineWidth(3.0)
        glColor3f(0.8, 0.2, 1.0)
        glBegin(GL_LINE_STRIP)
        sx, sy = random.uniform(-GRID, GRID), random.uniform(-GRID, GRID)
        sz = random.uniform(500, 900)
        glVertex3f(sx, sy, sz)
        for _ in range(4):
            sx += random.uniform(-150, 150)
            sy += random.uniform(-150, 150)
            sz -= random.uniform(80, 200)
            glVertex3f(sx, sy, sz)
        glEnd()
        glLineWidth(1.0)
        glPopMatrix()
...
def draw_floor():
    ...
    for i in range(-GRID, GRID, step):
        for j in range(-GRID, GRID, step):
            if (i//step+j//step)%2==0:
                c = (0.2, 0.45, 0.15) # Green Grass
            else:
                c = (0.35, 0.25, 0.15) # Brown Mud
                
            if is_rain_wave():
                c = tuple(x*0.75 for x in c)
    ...
    glLineWidth(2.0)
    glBegin(GL_LINES)
    glColor3f(0.15, 0.55, 0.15) # Blade color
    for gx, gy, gh in grass_blades:
        glVertex3f(gx, gy, 0)
        glVertex3f(gx, gy, gh)
    glEnd()
    glLineWidth(1.0)
...
KIND_HP    = {'normal':6,  'fast':3,   'tank':12,   'boss1':150, 'boss2':300} 
...
def draw_ult_effects():
    if ult_active:
        height = ult_anim_timer * 30  
        glPushMatrix()
        glTranslatef(player_pos[0], player_pos[1], height)
        glRotatef(180, 1, 0, 0)  
        
        glColor3f(0.8, 0.1, 0.1)
        gluCylinder(gluNewQuadric(), 0, 20, 40, 16, 1)
        glTranslatef(0, 0, 40)
        glColor3f(0.3, 0.3, 0.3)
        gluCylinder(gluNewQuadric(), 20, 20, 100, 16, 1)
        glTranslatef(0, 0, 100)
        glColor3f(1.0, 0.6, 0.0)
        gluCylinder(gluNewQuadric(), 15, 0, 50, 16, 1)
        glPopMatrix()

    for f in ult_fires:
        glPushMatrix()
        glTranslatef(f[0], f[1], 10)
        s = 1.0 + 0.3 * math.sin(f[2] * 0.5)
        glScalef(s, s, s)
...
def draw_hud():
    ...
    if ult_cooldown <= 0:
        draw_text(WIN_W - 250, WIN_H - 50, "[X] ULTIMATE READY!", color=(1.0, 0.2, 0.2))
    else:
        draw_text(WIN_W - 250, WIN_H - 50, f"ULT Cooldown: {ult_cooldown//60}s", color=(0.6, 0.6, 0.6))
...
def game_logic():
    ...
    if ult_active:
        ...
        if ult_anim_timer <= 0:
            ult_active = False
            for e in enemies[:]:
                ult_fires.append([e['x'], e['y'], 0])  
                e['hp'] -= (10 * BULLET_DAMAGE)  # Deal 20 damage
